...
//! Return const pointer to the resource named name.
 ...
 */
const LaxFiles::Attribute *LaidoutApp::Resource(const char *name)
{
	return resources.find(name);
}

//! Append to or replace a resource.
/*! resource must be a totally new instance, not something already contained in resources.
 ...
 *
 * \todo this resource business should probably be transferred to Laxkit.
 * \todo might be wise to have reference counted resources, but that's maybe getting too complicated
 *    for what these resources are meant for, that is: info used upon dialog creation
 */
int LaidoutApp::Resource(LaxFiles::Attribute *resource)
{
	if (!resource) return 1;
	Attribute *att=resources.find(resource->name);
