...
{
	if (!def && value) {
		def = value->GetObjectDef();
		if (def) def->inc_count();
	}
	if (!def) return 1;

	if (!def || !value) { // the value doesn't cough up an appropriate ObjectDef
		Utf8String str;
		str.Sprintf("Unknown display for value:\n%s", value->Id() ? value->Id() : "");
							
		AddWin(new MessageBar(this,NULL,value->Stylename()?value->Stylename():"unknown",MB_MOVE, 0,0,0,0, 0,str.c_str()));
		AddNull();
		delete[] blah;

	} else {
		last = NULL;
		MakeControls("def",def);
	}
...
void GenericValueDialog::MakeControls(const char *startext,ObjectDef *ndef)
{
	...
	for (int c=0; c<ndef->getNumFields(); c++) {	
		ObjectDef *sd = ndef->getField(c);
