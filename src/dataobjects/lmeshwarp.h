...
class LMeshWarpInterface : public LaxInterfaces::PatchInterface
{
 protected:
	MeshWarpNode *pnode;
 ...
 public:
	LMeshWarpInterface(LaxInterfaces::anInterface *nowner, int nid,Laxkit::Displayer *ndp);
	virtual ~LMeshWarpInterface();
	virtual const char *whattype() { return "MeshWarpInterface"; }
...
class MeshWarpNode : public ObjectFilterNode
{
	static SingletonKeeper keeper; //the def for the op enum

  public:
	static LaxInterfaces::MeshWarpInterface *GetMeshWarpInterface();

	bool render_preview;
	double render_dpi;
	LaxInterfaces::PatchData *transform;

	MeshWarpNode();
	virtual ~MeshWarpNode();
