...
//------------------------------- LMeshWarpInterface --------------------------------
/*! \class LMeshWarpInterface
 *
 * Use a PatchInterface to distort whatever.
 */


LMeshWarpInterface::LMeshWarpInterface(anInterface *nowner, int nid,Laxkit::Displayer *ndp)
  : MeshWarpInterface(nowner,nid,ndp)
{
	pnode = NULL;
}

LMeshWarpInterface::~LMeshWarpInterface()
{
	if (pnode) pnode->dec_count();
}


LaxInterfaces::anInterface *LMeshWarpInterface::duplicate(LaxInterfaces::anInterface *dup)
{
	if (dup==NULL) {
		MeshWarpInterface *p = new LMeshWarpInterface(NULL,id,NULL);
		dup = p;
		p->interface_flags = interface_flags;

    } else if (!dynamic_cast<LMeshWarpInterface *>(dup)) return NULL;

	return MeshWarpInterface::duplicate(dup);
}

int LMeshWarpInterface::UseThis(Laxkit::anObject *nobj,unsigned int mask)
{
	if (dynamic_cast<ObjectFilterInfo*>(nobj)) {
		dont_update_transform = true;

		ObjectFilterInfo *info = dynamic_cast<ObjectFilterInfo*>(nobj);
		MeshWarpNode *node = dynamic_cast<MeshWarpNode*>(info->node);
		if (!node) {
			DBG cerr << " Warning! no data coming into MeshWarpNode!"<<endl;
			return 1;
		}
...
//		if (!obj) {
//			DBG cerr << " Warning! no data for out of MeshWarpNode!"<<endl;
//			return 0;
//		}
		//------
	...
	return MeshWarpInterface::UseThis(nobj, mask);
}

/*! Underlying interface has updated transform, so this updates node state accordingly.
 */
void LMeshWarpInterface::Modified()
{
	if (!pnode) return;
...
SingletonKeeper MeshWarpNode::keeper;


/*! Static return for a MeshWarpInterface.
 */
LaxInterfaces::MeshWarpInterface *MeshWarpNode::GetMeshWarpInterface()
{
	MeshWarpInterface *interf = dynamic_cast<MeshWarpInterface*>(keeper.GetObject());
	if (!interf) {
		interf = new LMeshWarpInterface(NULL, getUniqueNumber(), NULL);
		interf->Id("meshwarpinterf");
		interf->interface_flags |= LaxInterfaces::MeshWarpInterface::PERSP_Dont_Change_Object;
...
MeshWarpNode::MeshWarpNode()
{
	makestr(Name, _("Mesh Warp Filter"));
	makestr(type, "Filters/MeshWarpFilter");

	render_preview = true;
	render_dpi = 300;

	transform = new PatchData;
...
//int MeshWarpNode::Connected(NodeConnection *connection)
//{
...
LaxInterfaces::anInterface *MeshWarpNode::ObjectFilterInterface()
{
	MeshWarpInterface *interf = GetMeshWarpInterface();
	return interf;
}

DrawableObject *MeshWarpNode::ObjectFilterData()
{
	NodeProperty *prop = FindProperty("out");
	return dynamic_cast<DrawableObject*>(prop->GetData());
}

NodeBase *MeshWarpNode::Duplicate()
{
	MeshWarpNode *node = new MeshWarpNode();
	node->DuplicateBase(this);
	node->DuplicateProperties(this);
	return node;
}

int MeshWarpNode::GetStatus()
{
	if (!transform->IsValid()) return -1;
	return NodeBase::GetStatus();
}

int MeshWarpNode::UpdateTransform()
{
...
// int MeshWarpNode::Mute(bool yes)
// {
// 	return NodeBase::Mute(yes);
// }

int MeshWarpNode::Update()
{
	Error(nullptr);
	...
	if (dynamic_cast<PathsData*>(torig)) {
		...
		DBG cerr << "mesh warp transform:"<<endl;
	...
	Error(_("Cannot apply mesh warp to that type!"));
	return -1;
