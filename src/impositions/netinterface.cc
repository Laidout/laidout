...
PanoramaInfo::PanoramaInfo(const char *file)
{}

PanoramaInfo::~PanoramaInfo()
{
	delete[] filename;
	delete[] polyhedron_file;
	delete[] sphere_file;
}
...
int PanoramaInfo::SavePolyptych(const char *saveto)
{
	...
	if (!isblank(polyhedron_file)) fprintf(f,"polyhedronfile %s\n",polyhedron_file);
	if (!isblank(sphere_file)) fprintf(f,"spherefile %s\n",sphere_file);
 ...
 */
NetInterface::NetInterface(Laxkit::Displayer *dp)
 	: anInterface(-1, dp)
{
	font = app->defaultlaxfont;
	font->inc_count();

	original_netimp = nullptr;
	current_netimp = nullptr;
	// original_netimp = new_net;
	// if (original_netimp) original_netimp->inc_count();
	// current_netimp = original_netimp;
	// if (current_netimp) current_netimp->inc_count();
...
void NetInterface::MakePaperInterface()
{
...
int NetInterface::InterfaceOn()
{
	...
	if (!paper_interface) {
		MakePaperInterface();
		if (current_netimp->papergroup) paper_interface->UseThis(current_netimp->papergroup);
	}
...
void NetInterface::Clear(LaxInterfaces::SomeData *d)
{
}
...
int NetInterface::GetDimensions(double &width, double &height)
{
	...
	else paper = current_netimp->papergroup->papers.e[0]->box->paperstyle;
...
{
	NetImposition *nimp = dynamic_cast<NetImposition*>(imp);
	if (!nimp) return 1;

	if (original_netimp != nimp) {
		if (original_netimp) original_netimp->dec_count();
		original_netimp = nimp;
		if (nimp) nimp->inc_count();
	}
	...
	if (current_netimp->papergroup) {
		if (!paper_interface) MakePaperInterface();
		paper_interface->UseThis(current_netimp->papergroup);
	}
...
int NetInterface::Refresh()
{
	...
	for (int c = 0; c < current_netimp->nets.n; c++) {
		...
		for (int c2 = 0; c2 < net->faces.n; c2++) {
			...
			if (face->tag != FACE_Potential) continue;
			if (poly && poly->faces.e[net->faces.e[c2]->original]->cache->facemode != 0) continue;
			// if (current_netimp->abstractnet) {
			// 	NetFace *aface = current_netimp->abstractnet->GetFace(net->faces.e[c2]->original, 1);
...
int NetInterface::LBUp(int x,int y,unsigned int state,const LaxMouse *mouse)
{
	...
	// int overlay = scanOverlays(x,y, nullptr,nullptr,&group);
	// if ((orig_group == NETI_TouchHelpers || orig_group == NETI_Paper) && overlayid > 0) {
	...
	// if (active_action == NETI_Unwrap) RBUp(x,y,0,mouse);
	// else if (active_action == NETI_Reseed) RBUp(x,y,ControlMask,mouse);
	return 0;
}
...
// 	//if (active_action!=NETI_Unwrap && active_action!=NETI_Reseed) return 0;
...
//		}
	...
	int current_action = active_action;
	// if (buttondown.isdown(0,RIGHTBUTTON)) current_action = NETI_Unwrap;


	if (current_action == NETI_Unwrap) {
		// Unwrap
		DBGL("must implement unwrap!!");
		// flatpoint p = screentoreal(x,y);
	...
	// if (current_action == NETI_None) {
	// 	if       ((state & LAX_STATE_MASK) == 0)                      current_action = NETI_Roll;
	// 	else if  ((state & LAX_STATE_MASK) == ShiftMask)              current_action = NETI_Rotate;
	// 	else if  ((state & LAX_STATE_MASK) == ControlMask)            current_action = NETI_Shift_Texture;
	// 	else if  ((state & LAX_STATE_MASK) ==(ShiftMask|ControlMask)) current_action = NETI_Rotate_Texture;
	// }

	currentface = -1;

	//  //shift-drag -- rotates camera around axis through viewer
	// if (current_action == NETI_Rotate) {
	...
	// } else if (current_action==NETI_Roll) { //rotate current thing
	// 	flatpoint d,p=flatpoint(x,y);
	// 	d=p-leftb;
	...
	// } else if (current_action==NETI_Rotate_Texture) { //rotate texture around z
	// 	flatpoint d,p=flatpoint(x,y);
	// 	d=p-leftb;
	...
	// } else if (current_action==NETI_Shift_Texture) { //rotate texture
	// 	flatpoint d,p=flatpoint(x,y);
	// 	d=p-leftb;
...
{
	if (!current_netimp) return -1;
	return removeNet(current_netimp->nets.findindex(net));
}

//! Remove net and set all faces to be on hedron (facemode==0).
int NetInterface::removeNet(int netindex)
{
	if (netindex < 0 || netindex >= current_netimp->nets.n) return 1;
	Net *net = current_netimp->nets.e[netindex];
	if (currentnet == net) {
	...
	DBGL("removeNet() reset " << n << " faces");
	current_netimp->nets.remove(netindex);
	return 0;
}
...
{
	index = -1;
	if (!current_netimp || !paper_interface) {
		return 0;
	}

	index = paper_interface->scan(x,y);
	if (index >= 0) return NETI_Paper;
	return NETI_None;
}

/*! Return index in currentnet->faces, or -2 over paper (if any) or -1 for not over anything.
 */
int NetInterface::findCurrentPotential(const flatpoint &p, int &neti)
{
	int index = -1;
	...
	if (current_netimp->nets.e[neti]->faces.e[index]->tag != FACE_Potential) return -1;
 ...
 */
int NetInterface::findCurrentFace(const flatpoint &p, int &neti)
{
	int index = -1;
	...
	if (current_netimp->nets.e[neti]->faces.e[index]->tag != FACE_Actual) return -1;
	// if (poly->faces.e[currentnet->faces.e[index]->original]->cache->facemode != 0) return -1;
...
Net *NetInterface::findNet(int id)
{
	if (id < 0) id = -id;
	for (int c = 0; c < current_netimp->nets.n; c++)
		if ((int)current_netimp->nets.e[c]->object_id == id)
			return current_netimp->nets.e[c];
	return nullptr;
}
 ...
 */
int NetInterface::scanOverlays(int x,int y, int *action,int *index,int *group)
{
	return 0;
...
int NetInterface::AddNet(Net *net)
{
	...
	current_netimp->nets.push(newnet);
	newnet->dec_count();
	needtodraw = 1;
	return 1;
}
...
int NetInterface::InstallPolyhedron(Polyhedron *ph)
{
	...
	current_netimp->nets.flush();
	if (currentnet) currentnet->dec_count();
	currentnet = nullptr;

	current_netimp->InstallAbstractNet(ph);
...
int NetInterface::InstallPolyhedron(const char *file)
{
	...
	if (currentnet) currentnet->dec_count();
	currentnet = nullptr;
	current_netimp->InstallAbstractNet(poly);
	poly->dec_count();
...
int NetInterface::Event(const EventData *data,const char *mes)
{
	...
	if (!strcmp(mes,"new sphere image")) {
		// const StrEventData *s=dynamic_cast<const StrEventData*>(data);
		// if (!s) return 1;
		// installSpereImage(s->str);
		// return 0;
		PostMessage("Need to implement loading sphere image!!");
		return 0;
	}
	...
	// 	DBGE("**** must implement background rendering, and multinet output!!");
	...
	// 			 outfile,       // Say filebase="blah", files will be blah000.png, ...this creates default filebase name
	...
	return anInterface::Event(data, mes);
}
...
Laxkit::ShortcutHandler *NetInterface::GetShortcuts()
{
	...
	sc->Add(NETI_Paper,          'p',0,0,           _("TogglePapers"), _("Toggle paper overlay"),nullptr,0);
	sc->Add(NETI_LoadPolyhedron, 'p',ControlMask,0, _("Hedron"),       _("Load new polyhedron"),nullptr,0);
	sc->Add(NETI_LoadSphereImage,'i',ControlMask,0, _("SphereImage"),  _("Load sphere image"),nullptr,0);
...
int NetInterface::PerformAction(int action)
{
	...
	} else if (action == NETI_LoadSphereImage) {
		// change image
		app->rundialog(new FileDialog(nullptr,_("New sphere image"),_("New sphere image"),
									ANXWIN_REMEMBER,
									0,0,800,600,0, object_id,"new sphere image",
	...
	// 	return 0;
		...
		// total unwrap
		while (current_netimp->nets.n > 1) current_netimp->nets.remove(current_netimp->nets.n - 1);
		if (current_netimp->nets.n == 0) {
			Net *net     = new Net;
			net->_config = 1;
			net->Basenet(current_netimp->abstractnet);
			net->Unwrap(-1, 0);
			if (poly) poly->faces.e[0]->cache->facemode = -net->object_id;
			net->info = 0;

			current_netimp->nets.push(net);
			net->dec_count();
		}
		current_netimp->nets.e[0]->TotalUnwrap();
		if (poly) {
			for (int c = 0; c < poly->faces.n; c++) {
				if (c == current_netimp->nets.e[0]->info) poly->faces.e[c]->cache->facemode = -current_netimp->nets.e[0]->object_id;
				poly->faces.e[c]->cache->facemode = current_netimp->nets.e[0]->object_id;
			}
		}
		needtodraw = 1;
		return 0;
	...
	// 	currentface++;
	// 	if (currentface >= current_netimp->poly->faces.n) currentface = 0;
	// 	needtodraw = 1;
	// 	return 0;
