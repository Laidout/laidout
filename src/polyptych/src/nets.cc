...
{
	if (linestyle && l.linestyle) *linestyle = *l.linestyle;  //***todo: this is maybe problematic
	tag = l.tag;
	lineinfo = l.lineinfo;
	style_hint = l.style_hint;
...
int NetLine::Set(const char *d,LineStyle *ls)
{
	...
	if (points) delete points;
	points = SvgToCoordinate(d,0,nullptr,nullptr);
	return points ? 0 : 1;
}
...
void NetLine::dumpOut(FILE *f,int indent, int what)
{
	...
	if (what==-1) {
		...
		fprintf(f,"%slineinfo     # Whether line is autogenerated (0) or user generated (1)\n",spc);
		fprintf(f,"%sstyle_hint   # Hint for style of line\n",spc);
		if (linestyle) linestyle->dump_out(f,indent+2,-1,nullptr);
		else {
			LineStyle line;
			line.dump_out(f,indent+2,-1,nullptr);
		}
		return;
	}
	if (points) {
	...
	if (linestyle) {
		...
		linestyle->dump_out(f,indent+2,0,nullptr);//note: context is nullptr, is this ok?
	}
	fprintf(f,"%slineinfo  %d\n",spc, lineinfo);
	fprintf(f,"%sstyle_hint %d\n",spc, style_hint);
}

/*! If val!=nullptr, then the att was something like:
 * <pre>
 *   line M 1 1 L 0 4
 *     (other stuff)..
 * </pre>
...
void NetLine::dumpInAtts(Laxkit::Attribute *att, const char *val,int flag)
{
	...
	if (val) {
		if (points) delete points;
		points = SvgToCoordinate(val,0,nullptr,nullptr);
	}
	char *name,*value;
	for (c=0; c<att->attributes.n; c++) {
		...
		if (!strcmp(name,"points")) {
			if (points) delete points;
			points=SvgToCoordinate(value,4,nullptr,nullptr);
			//cout <<"**** rewrite to watch for LOOP_TERMINATOR stuff!!"<<endl;
		} else if (!strcmp(name,"linestyle")) {
			if (!linestyle) {
				linestyle=new LineStyle();
			}
			linestyle->dump_in_atts(att->attributes.e[c],flag,nullptr);//note: context is nullptr, is this ok?
		} else if (!strcmp(name,"tag")) {
			IntAttribute(value,&tag);
		} else if (!strcmp(name, "lineinfo")) {
			IntAttribute(value, &lineinfo);
		} else if (!strcmp(name, "style_hint")) {
			IntAttribute(value, &style_hint);
		}
	}
}
...
const NetFaceEdge &NetFaceEdge::operator=(const NetFaceEdge &e)
{
	...
	if (points) { delete points; points = nullptr; }
	Coordinate *p = e.points, *p2 = nullptr;
	while (p) {
		if (!p2)
			p2 = points = new Coordinate(*p);
		else {
 ...
 *
 * The default (matrix==nullptr or is identity) is for the origin to be at the beginning
...
//{***
//	if (list && points) { delete[] points; points=nullptr; }
//	if (link && facelink) { delete[] facelink; facelink=nullptr; }
//	int n;
//	if (link) IntListAttribute(link,&facelink,&n);
//	else n=np;
//	if (list) IntListAttribute(list,&points,&np);
//	if (n!=np) {
//		if (facelink) { delete[] facelink; facelink=nullptr; }
//		facelink=new int[np];
		...
		if (!strcmp(name,"matrix")) {
			...
			for (int c2=0; c2<att->attributes.e[c]->attributes.n; c2++) {
				...
				if (!strcmp(name,"tooriginal")) { //could be "3" or "3.5"
					...
					edge->points=SvgToCoordinate(value,0,nullptr,nullptr);
				} else if (!strcmp(name,"potential")) {
...
//! Return the file associated with this net, if any. Default is return nullptr.
...
BasicNet::BasicNet(const char *nname)//nname=nullptr
{
	makestr(name,nname);
}
...
void Net::clear()
{
	if (netname) { delete[] netname; netname=nullptr; }
...
void Net::FindBBox()
{
	...
	for (int c=0; c<lines.n; c++) {
		...
		if (cc) do {
			addtobounds(cc->fp);
			cc=cc->next;
			if (cc==lines.e[c]->points) cc=nullptr;
		} while (cc);
	}
}
...
void  Net::dump_in_atts(Laxkit::Attribute *att,int flag,Laxkit::DumpContext *context)
{
	...
	for (c=0; c<att->attributes.n; c++) {
		...
		if (!strcmp(name,"name")) {
			...
			//	baseref=value+4;
				...
				basenet=loadBaseNet(value+5,nullptr);//***ignores error string return
			}
	...
	DBG dump_out(stderr,0,0,nullptr);
...
AbstractNet *Net::loadBaseNet(const char *filename,char **error_ret)
{
	...
	net->dump_in_atts(&att,0,nullptr);
	return net;
}
...
{ //***
	return 1;
//	if (file_exists(filename,1,nullptr)!=S_IFREG) {
...
//	}
//	setlocale(LC_ALL,"C");
//	if (error_ret) *error_ret=nullptr;
//
//	int e=0, 
//		numv=0,
//		numf=0,
...
//		int *i=new int[3];
//		double p[4];
//		c=IntListAttribute(line,i,3,nullptr);
//		if (c!=3) { e=1; break; }
//		numv=i[0];
//		numf=i[1];
...
//			if (c<=0) { e=1; break; }
//			
//			c=DoubleListAttribute(line,p,3,nullptr);
//			if (c!=3) { e=1; break; }
//
...
//			if (c<=0) { e=1; break; }
//			
//			c=IntListAttribute(line,&i,nullptr,nullptr);
...
int Net::Anchor(int basenetfacei)
{
	...
	if (faces.n && basenetfacei>=0) {
		int c=findOriginalFace(basenetfacei,1,-1,nullptr);
	...
	if (faces.n && basenetfacei<0) {
		for (basenetfacei=0; basenetfacei<basenet->NumFaces(); basenetfacei++) {
			if (findOriginalFace(basenetfacei,1,-1,nullptr)==0) break;
		}
...
int Net::addPotentialsToFace(int facenum)
{
	...
	for (int c=0; c<faces.e[facenum]->edges.n; c++) {
		...
		if (findOriginalFace(faces.e[facenum]->edges.e[c]->tooriginal,1,-1,nullptr)==1) {
...
/*! Find folds, and mark their edges as EDGE_Fold.
...
int Net::rebuildLines()
{
	...
	int previous_edge_info = Net::EDGE_Unknown;
	int line_start;
	...
	for (int c = 0; c < faces.n; c++) {
		...
		nl = nlp = nullptr;
		line_start = 0;
		previous_edge_info = Net::EDGE_Unknown;
		...
		for (int c2 = 0; c2 < faces.e[c]->edges.n; c2++) {
			...
			if (visited.Contains(edge)) {
				...
				if (nl) {
					// not closed, but we use current edge first point as end point
					nlp->next = new Coordinate();
					nlp->next->prev = nlp;
					nlp = nlp->next;
					...
					l->style_hint = previous_edge_info;
					lines.push(l,1);
				}
				nl = nlp = nullptr;
				previous_edge_info = 0;
				line_start = c2;
				continue;

			} else if (edge->info != previous_edge_info) {
				// different line style, need to start a new one, and add current to lines
				// we assume it does not close the line
				if (nl) {
					// not closed, but we use current edge first point as end point
					nlp->next = new Coordinate();
					nlp->next->prev = nlp;
			...
			if (edge->toface >= 0) {
				visited.push(faces.e[edge->toface]->edges.e[edge->tofaceedge], 0);
			}
			
			// add edge points to current line
			if (!nl) {
				line_start = c2;
				previous_edge_info = edge->info;
			}
			...
			if (nl && c2 == faces.e[c]->edges.n-1) {
				if (line_start == 0) {
					// close line
					nlp->next = nl;
					nl->prev = nlp;
				} else {
					// we need to use the initial point from the first edge
					p = faces.e[c]->edges.e[0]->points;
					nlp->next = new Coordinate();
					nlp->next->prev = nlp;
					nlp = nlp->next;
					if (faces.e[c]->matrix) nlp->fp = transform_point(faces.e[c]->matrix,p->fp);
...
int Net::Unwrap(int netfacei,int atedge)
{
	...
	if (netfacei<0) {
		...
		if (findOriginalFace(atedge,1,0,nullptr)==1) return 0;
