 ...
 *
 * The face connecting to it must actually be connecting to it physically in the flattened Net. If the original face is
 ...
 *   - connected and adjacent line:  a fold
 *   - connected and adjacent line: soft cut?
 *   - disconnected line: hard cut?
 *   
 */
int Net::rebuildLines()
{
	...
	Coordinate *p, *nl, *nlp=nullptr;
	PtrStack<NetFaceEdge> visited;
	...
	for (int c = 0; c < faces.n; c++) {
		...
		for (int c2 = 0; c2 < faces.e[c]->edges.n; c2++) {
			...
			// //-------------------
			// if (visited.Contains(edge)) { // *** todo: else if edge is different style
			// 	// if we have an edge, then finish it
			// 	***
			// 	nl = nlp = nullptr;
			// } else {
			// 	visited.push(edge,0);
			// 	// also push edge from physically connected face
			// 	if (edge->toface >= 0 && edge->tofacedge >= 0) {
			// 		visited.push(faces.e[edge->toface]->edges.e[edge->tofaceedge]);
			// 	}
			// }
			//-------------------
			while (p) {
